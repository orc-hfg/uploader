...
test.describe('Project page', () => {
	...
	test('should show project page correctly', async ({ page, makeAxeBuilder }) => {
		await expect(page).toHaveTitle('Test collectionId collection-id-1 / metaKeyId madek_core:title Content – Uploader');
		await expect(page.getByRole('heading', { name: 'Test collectionId collection-id-1 / metaKeyId madek_core:title Content' })).toBeVisible();
	...
	test.skip('should display projects with images correctly', async ({ page }) => {
	...
	test.skip('should show fallback message when project has no image', async ({ page }) => {
...
test.describe('Projects page (English locale)', () => {
	...
	test.skip('should load different project titles for English locale', async ({ page }) => {

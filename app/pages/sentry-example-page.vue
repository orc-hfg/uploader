...
<script setup lang="ts">
	...
	const appLogger = createAppLogger('Page: sentry-example-page');
	...
	onMounted(async () => {
		try {
			// See: https://github.com/getsentry/sentry-javascript/issues/16226
			const result = await Sentry.diagnoseSdkConnectivity();
			isConnected.value = result !== 'sentry-unreachable';

			if (result === undefined) {
				appLogger.info('The initial health-check request purposely triggers a 400 Bad Request. This only verifies that the Sentry endpoint is reachable. Actual error events will still be sent successfully.');
			}
		}
		catch {
...
<template>
	<div>
		<main>
			...
			<p class="description">
				Click the button below, and view the sample error on the Sentry <a target="_blank" rel="noopener noreferrer" href="https://rene-zschoch.sentry.io/issues/?project=4509604854104144">Issues Page</a>.
				For more details about setting up Sentry, <a target="_blank" rel="noopener noreferrer" href="https://docs.sentry.io/platforms/javascript/guides/nuxt/">read our docs</a>.
			</p>
			<button
				type="button"
